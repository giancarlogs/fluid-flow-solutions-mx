  ...
  return <section className="section-padding">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {solutions.map(solution => <Card key={solution.id} className={`border-none shadow-md transition-all duration-300 ${expandedId === solution.id ? 'ring-2 ring-espaf-yellow transform scale-[1.02]' : 'hover:shadow-lg hover:-translate-y-1'}`}>
              <div className={`h-2 bg-gradient-to-r ${solution.id === 1 ? 'from-espaf-blue-dark to-espaf-blue-medium' : solution.id === 2 ? 'from-espaf-blue-medium to-espaf-teal' : 'from-espaf-teal to-espaf-blue-dark'}`}></div>
              
              <CardHeader className="flex flex-row items-center gap-4 pt-8">
                <div className={`p-3 rounded-full ${solution.id === 1 ? 'bg-espaf-blue-dark text-white' : solution.id === 2 ? 'bg-espaf-blue-medium text-white' : 'bg-espaf-teal text-white'}`}>
              ...
              <CardContent className="pb-4">
                ...
                {expandedId === solution.id && <div className="animate-fade-in mt-4">
                    <h4 className="font-semibold text-espaf-blue-dark mb-2">
                      Nuestros servicios incluyen:
                    </h4>
                    <ul className="list-disc list-inside text-gray-600 space-y-1">
                      {solution.details.map((detail, idx) => <li key={idx}>{detail}</li>)}
                    </ul>
                  </div>}
              </CardContent>
              
              <CardFooter>
                <Button variant="ghost" className="text-espaf-blue-medium hover:text-espaf-yellow hover:bg-transparent p-0" onClick={() => setExpandedId(expandedId === solution.id ? null : solution.id)}>
        ...
        <div className="bg-espaf-gray/20 rounded-lg p-8 md:p-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="hidden md:block">
              <img alt="Enfoque ESPAF" src="/lovable-uploads/49541bf3-f606-4d92-b788-a7245f7c0fdb.jpg" className="rounded-lg shadow-lg object-fill" />
            </div>
          </div>
        </div>
      </div>
